...
The derivation of the recursive forms of $f(x)$, $g(x)$ and the coefficients $c_{k}$ are given in appendix \ref{app:recursion}. Note that $c_{k}$ benefits hugely from recursive computation as it prevents us having to calculate $216^{k}$, requiring us to merely multiply successive terms by $1/216$. TODO: mention how many terms the recursive term can handle vs. the regular computation.
